...
func ReturnFirst(w http.ResponseWriter, r *http.Request) {
  ...
  default:
	...
	if isCTSURN(requestUrn) != true {
		...
		result := NodeResponse{RequestUrn: requestUrn, Status: "Exception", Message: message}
	...
	default:
		...
		result = NodeResponse{RequestUrn: requestUrn,
			Status: "Success",
	...
	default:
		...
		case contains(RequestedWork.URN, requestUrn):
			...
			case requestedIndex == 0:
				result = NodeResponse{RequestUrn: requestUrn, Status: "Success", Nodes: []Node{}}
	...
	default:
      ...
      result = URNResponse{RequestUrn: requestUrn, Status: "Success", URN: range_urn}
		...
		default:
			switch {
			case contains(RequestedWork.URN, requestUrn):
				result = URNResponse{RequestUrn: requestUrn, Status: "Success", URN: []string{requestUrn}}
  ...
  default:
	...
	default:
		...
		case level1contains(RequestedWork.URN, requestUrn):
			...
			result = NodeResponse{RequestUrn: requestUrn, Status: "Success", Nodes: matchingNodes}
		case level2contains(RequestedWork.URN, requestUrn):
			var matchingNodes []Node
			var match []bool
			for i := range RequestedWork.URN {
		...
		case isRange(requestUrn):
			...
			result = NodeResponse{RequestUrn: requestUrn, Status: "Success", Nodes: rangeNodes}
		default:
			message := "Could not find node to " + requestUrn + " in source."
			result = NodeResponse{RequestUrn: requestUrn, Status: "Exception", Message: message}
		}
